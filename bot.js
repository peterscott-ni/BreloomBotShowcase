...
var aviNormal = require("fs").readFileSync(
  "BreloomBotSmallSmile.png",
  "base64"
);
        ...
        if (member.id == result[i].UserID) {
          bot.addToRole({
            ...
            function (err) {
              ...
              sendToServer(
                testChannelBugs,
                "Error in function: guildMemberAdd"
              );
              sendToServer(testChannelBugs, err);
            };
        }
      }
    } else sendToServer(testChannelBugs, "UserRejoinedServer encountered an error: no roles found for user: " + member.nick);
  });
});
...
function checkKeywords(message, user, channelID) {
  if (user.toLowerCase() == "admin") {
    for (let i in keywords.users) {
      // next user
      let keyWordFound = false;
      for (let j = 0; j < keywords.users[i].length; j++) {
        // next keyword
        if (!keyWordFound) {
          // stop after first matching keyword found
...
bot.on("message", function (user, userID, channelID, message, evt) {
  ...
  if (
    message.toLowerCase().includes("clarice, what's") ||
    message.toLowerCase().includes("clarice, what is") ||
    message.toLowerCase().includes("clarice what's") ||
    message.toLowerCase().includes("clarice what is") ||
    message.toLowerCase().includes("clarice show me") ||
    message.toLowerCase().includes("clarice, show me")
  ) {
    var input = "";
    ...
    client.search(input).then((images) => {
      if (images.length > 0) {
        let url =
          images[Math.floor(Math.random() * Math.min(images.length, 10))].url;
  ...
  if (message.toLowerCase().includes("what does pot of greed do")) {
    sendToServer(
      channelID,
      "**POT OF GREED ALLOWS ME TO DRAW TWO MORE CARDS FROM MY DECK AND ADD THEM TO MY HAND**"
    );
  }
  ...
  if (/*channelID != "442322003001868289" || message.includes("prune")*/ true) {
    // avoids posting to srs-bsns, unless pruning
    if (
      message.toLowerCase().includes("clarice") &&
      message.toLowerCase().includes("play despacito")
    ) {
      sendToServer(
        channelID,
        "**Des...pa...*cito*** :musical_note:\nhttps://youtu.be/kJQP7kiw5Fk"
      );
    }
    if (
      message.toLowerCase().includes("cincinno") ||
      message.toLowerCase().includes("cinncino")
    ) {
      sendToServer(
        channelID,
        'Bibs wants you to know that it\'s spelled "Cinccino", ' +
          userNickname(user) +
          "."
      );
      return;
    ...
    if (
      message.toLowerCase().includes("mincinno") ||
      message.toLowerCase().includes("minncino")
    ) {
      sendToServer(
        channelID,
        'Bibs wants you to know that it\'s spelled "Minccino", ' +
          userNickname(user) +
          "."
      );
      return;
    ...
    if (
      message.toLowerCase().includes("goddammit admin") &&
      userID == "174871903800918016"
    )
      // bibs' user ID
    ...
    if (
      ...
      (message.toLowerCase().includes("inferior") ||
        message.toLowerCase().includes("silly") ||
        message.toLowerCase().includes("dumb") ||
        message.toLowerCase().includes("defunct") ||
        message.toLowerCase().includes("pathetic") ||
        message.toLowerCase().includes("inadequate") ||
        message.toLowerCase().includes("stupid") ||
        message.toLowerCase().includes("trash") ||
        message.toLowerCase().includes("garbage") ||
        message.toLowerCase().includes("useless"))
    )
      ...
      switch (cmd) {
        ...
        case "goddammitadmin": // GODDAMMIT admin
          if (user != "Clarice")
            sendToServer(
              ...
              "Hey, admin! " +
                userNickname(user) +
                goddammitadminGeneral(channelID, user),
              true
            );
          break;
        case "testgirl":
          ...
          setTimeout(
            ...
            500,
            channelID,
            goodGirlResponse() + userNickname(user) + "!"
          );
          break;
        ...
        case "hangmancancel": // CANCEL ACTIVE HANGMAN GAME
          if (hangmanActive)
            sendToServer(
              channelID,
              "The active game of hangman has been cancelled."
            );
          else
            sendToServer(
              channelID,
              "I tried to cancel it, but it turns out there wasn't one anyway."
            );
          resetGame();
          break;
        case "helix": // CONSULT THE MIGHTY HELIX
          ...
          if (message == "*") {
            sendToServer(
              channelID,
              "Perhaps these words of wisdom will answer your question."
            );
            setTimeout(sendToServer, 500, channelID, "!pt");
          } else sendToServer(channelID, message);
          break;
        case "help": // HELP TEXT
            ...
            function (err, res) {
              try {
                if (res.author.id == "512711179752177675") {
                  ...
                  sendToServer(
                    channelID,
                    "I can only delete messages sent by me!"
                  );
                }
              } catch (e) {
                sendToServer(
                  channelID,
                  "You need to follow the command with the message ID! Only my messages can be deleted."
                );
              }
            }
          );
        ...
        case "remove":
          if (userID == adminID) {
            ...
            sendToServer(
              channelID,
              "Only admin can use this command, ask him!"
            );
          }
          break;
        ...
        case "uptime":
          sendToServer(
            channelID,
            "I've been active for: " + uptimeFormat(process.uptime())
          );
          break;
        case "testaddrole":
          bot.addToRole({
...
function addNewMemeCheck(sourceChannelID, message) {
  ...
  if (hasImage) {
    let queryResetAutoIncrement =
      "ALTER TABLE pokemon." + memeTable + " AUTO_INCREMENT = 1;";
...
function createPokemon(channelID, message, user) {
  ...
  switch (Math.floor(Math.random() * 20)) {
    case 0:
      ...
      break;
    case 1:
      nameGenerator = "cvcvhdv";
      break; // pikachu
    case 2:
      nameGenerator = "vhhdvc";
      break; // aggron
    case 3:
      nameGenerator = "cvccvccv";
      break;
    case 4:
      nameGenerator = "hdvccv";
      break;
    case 5:
      nameGenerator = "vccvcvv";
      break;
    case 6:
      nameGenerator = "cvcvcvcv";
      break; // giratina
    case 7:
      nameGenerator = "cvcvccvdc";
      break;
    case 8:
      nameGenerator = "vccvcvvc";
      break;
    case 9:
      nameGenerator = "hdvccvc";
      break;
    case 10:
      nameGenerator = "cvcvvc";
      break;
    case 11:
      nameGenerator = "hdvcvcvcc";
      break;
    case 12:
      nameGenerator = "cvvccv";
      break;
    case 11:
      nameGenerator = "cvdcc";
      break;
    case 12:
      nameGenerator = "hdvvcc";
      break;
    case 13:
      nameGenerator = "cvccvcvc";
      break; // magneton
    case 14:
      nameGenerator = "hdvccvvcv";
      break; // blastoise
    case 15:
      nameGenerator = "vcvc";
      break; // aron
    case 16:
      nameGenerator = "hdvcvvc";
      break; // tropius
    case 17:
      nameGenerator = "vddvcvcv";
      break; // illumise
    case 18:
      nameGenerator = "hdvccvc";
      break; // blissey
    case 19:
      nameGenerator = "hdvccvdc";
      break; // swampert
  }
  for (var i = 0; i < nameGenerator.length; i++) {
    if (nameGenerator.substring(i, i + 1) == "v")
      ...
      name += softConsonants[Math.floor(Math.random() * softConsonants.length)];
  }
  ...
  if (type1 == "Ghost" || type2 == "Ghost") weight *= 0.05;
  if (
    type1 == "Steel" ||
    type2 == "Steel" ||
    type1 == "Rock" ||
  ...
  while (bst < 425 || bst > 575) {
    switch (
      Math.floor(Math.random() * 10) // bmin, bmax, rmin, rmax, n
    ) {
      case 0:
        bst = randomGenerator(210, 720, 210, 720, 1);
        break; // flat baseline
      case 3:
      case 4:
        bst = randomGenerator(250, 370, 250, 370, 3);
        break; // initial peak in 300's
      case 5:
      case 6:
      case 7:
      case 8:
        bst = randomGenerator(370, 580, 370, 580, 3);
        break; // main peak in 400's
      default:
        bst = randomGenerator(0, 700, 400, 620, 3);
        break; // gradual descent towards higher BSTs
    }
  }

  if (Math.random() > 0.125) while (bst % 5 != 0) bst--;
  var bstLeft = bst;
  ...
  var sqlAbility =
    "SELECT identifier FROM Abilities WHERE id = " +
    (Math.floor(Math.random() * 233) + 1);
  if (Math.random() < 0.7)
...
function defineWord(channelID, message) {
  ...
  for (var i = 2; i < message.split(" ").length; i++) {
    ...
    if (message.split(" ")[i] != null) input += " " + message.split(" ")[i]; // add word to input
  }

  unirest
    .get("https://wordsapiv1.p.rapidapi.com/words/" + input + "/definitions")
    .header(
      "X-Mashape-Key",
      "568bc57230msh67e37b95b5b9316p157f6cjsn68790e0251d8"
    )
      ...
      for (var i = 0; i < result.body.definitions.length; i++) {
        try {
          outputString +=
            ...
            (i + 1) +
            ". *(" +
            result.body.definitions[i].partOfSpeech.substring(0, 1) +
            ".)  " +
            toTitleCase(result.body.definitions[i].definition.substring(0, 1)) +
            result.body.definitions[i].definition.substring(1) +
            "*";
        } catch (e) {
          outputString +=
            "\n" +
  ...
  function defineAlternative() {
    var sqlDefine =
      'SELECT word, type, `definition` FROM Dictionary WHERE word="' +
      input +
      '"';
    con.query(sqlDefine, function (err, result, fields) {
      if (result.length != 0) {
        ...
        definitionString = definitionString.replace(/\@/g, ",");
        sendToServer(channelID, definitionString);
      } else sendToServer(channelID, "No word was found. Try again?");
    });
  }
}
...
function details(channelID, message) {
  var input = message.split(" ")[1]; // input from discord
  for (var i = 2; i < message.split(" ").length; i++) {
    if (message.split(" ")[i] != null) input += " " + message.split(" ")[i];
  }
  ...
  if (isNaN(input)) {
    var basics =
      'SELECT identifier, id, height, weight, species_id, classification FROM Pokemon WHERE Pokemon.identifier="' +
      input +
      '"';
  } else
    var basics =
      "SELECT identifier, id, height, weight, species_id, classification FROM Pokemon WHERE Pokemon.species_id=" +
      input +
      ";";
  ...
  if (isNaN(input))
    var stats =
      'SELECT PokemonStats.base_stat FROM Pokemon INNER JOIN PokemonStats ON Pokemon.id = PokemonStats.pokemon_id WHERE Pokemon.identifier="' +
      input +
      '"';
  else
    var stats =
      "SELECT PokemonStats.base_stat FROM Pokemon INNER JOIN PokemonStats ON Pokemon.id = PokemonStats.pokemon_id WHERE Pokemon.species_id=" +
      input +
      ";";
  ...
  if (isNaN(input))
    var types =
      'SELECT Types.identifier FROM Pokemon INNER JOIN PokemonTypes ON Pokemon.id = PokemonTypes.pokemon_id INNER JOIN Types ON PokemonTypes.type_id = Types.id WHERE Pokemon.identifier="' +
      input +
      '"';
  else
    var types =
      "SELECT Types.identifier FROM Pokemon INNER JOIN PokemonTypes ON Pokemon.id = PokemonTypes.pokemon_id INNER JOIN Types ON PokemonTypes.type_id = Types.id WHERE Pokemon.species_id=" +
      input +
      ";";
  con.query(types, function (err, result, fields) {
    if (result.length != 0) {
      type1 = result[0].identifier;
      if (result.length > 1) type2 = result[1].identifier;
      typesComplete = true;
      finishString();
    }
  });
  if (isNaN(input))
    var abilities =
      'SELECT Abilities.identifier FROM Pokemon INNER JOIN (Abilities INNER JOIN PokemonAbilities ON Abilities.id = PokemonAbilities.ability_id) ON Pokemon.id = PokemonAbilities.pokemon_id WHERE (Pokemon.identifier)="' +
      input +
      '" ORDER BY slot';
  else
    var abilities =
      "SELECT Abilities.identifier FROM Pokemon INNER JOIN (Abilities INNER JOIN PokemonAbilities ON Abilities.id = PokemonAbilities.ability_id) ON Pokemon.id = PokemonAbilities.pokemon_id WHERE (Pokemon.id)=" +
      input +
      " ORDER BY slot";
  ...
  function finishString() {
    ...
    if (type2 != "") typeString += " / " + type2;

    var abilityString = ability1;
    if (ability2 != "") {
      abilityString += " / " + ability2;
      if (ability3 != "") abilityString += " / " + ability3;
    }
  ...
  function abilityCheck() {
    var sqlAbility =
      'SELECT identifier, flavor_text, generation_id FROM Abilities WHERE identifier="' +
      input +
      '"';
    con.query(sqlAbility, function (err, result, fields) {
      try {
        if (result.length != 0) {
          ...
          var formattedString =
            new String(
              "**" + result[0].identifier + "** (ability)" + "\n" + flavorText
            ) +
            "\nGen: " +
            result[0].generation_id;
          sendToServer(channelID, formattedString);
        } else moveCheck();
      } catch (err) {
  ...
  function moveCheck() {
    var sqlAbility =
      'SELECT Moves.identifierM, Moves.flavor_text, Types.identifier, Moves.power, Moves.accuracy, Moves.pp, Moves.priority, Moves.damage_class_id FROM Types INNER JOIN Moves ON Types.id = Moves.type_id WHERE (Moves.identifierM)="' +
      input +
      '"';
    con.query(sqlAbility, function (err, result, fields) {
      try {
        if (result.length != 0) {
          ...
          var formattedString = new String(
            "**" +
              result[0].identifierM +
              "**\nType: " +
              result[0].identifier +
              "    Category: " +
              damageCategory +
              "\nPower: " +
              (result[0].power == null ? "--" : result[0].power) +
              "    PP: " +
              result[0].pp +
              "    Accuracy: " +
              (result[0].accuracy == null ? "--" : result[0].accuracy) +
              "    Priority: " +
              result[0].priority +
              "\n" +
              flavorText
  ...
  function itemCheck() {
    var sqlItem =
      'SELECT identifier, prose FROM Items WHERE identifier="' + input + '"';
  ...
  function natureCheck() {
    switch (message.split(" ")[1].toLowerCase()) {
      case "hardy":
        response = "**Hardy nature**\nNeutral";
        break;
      case "lonely":
        response = "**Lonely nature**\n+Att -Def";
        break;
      case "brave":
        response = "**Brave nature**\n+Att -Spe";
        break;
      case "adamant":
        response = "**Adamant nature**\n+Att -SpA";
        break;
      case "naughty":
        response = "**Naughty nature**\n+Att -SpD";
        break;
      case "bold":
        response = "**Bold nature**\n+Def -Att";
        break;
      case "docile":
        response = "**Docile nature**\nNeutral";
        break;
      case "relaxed":
        response = "**Relaxed nature**\n+Def -Spe";
        break;
      case "impish":
        response = "**Impish nature**\n+Def -SpA";
        break;
      case "lax":
        response = "**Lax nature**\n+Def -SpD";
        break;
      case "timid":
        response = "**Timid nature**\n+Spe -Att";
        break;
      case "hasty":
        response = "**Hasty nature**\n+Spe - Def";
        break;
      case "serious":
        response = "**Serious nature**\nNeutral";
        break;
      case "jolly":
        response = "**Jolly nature**\n+Spe -SpA";
        break;
      case "naive":
        response = "**Naive nature**\n+Spe -SpD";
        break;
      case "modest":
        response = "**Modest nature**\n+SpA -Att";
        break;
      case "mild":
        response = "**Mild nature**\n+SpA -Def";
        break;
      case "quiet":
        response = "**Quiet nature**\n+SpA -Spe";
        break;
      case "bashful":
        response = "**Bashful nature**\nNeutral";
        break;
      case "rash":
        response = "**Rash nature**\n+SpA - SpD";
        break;
      case "calm":
        response = "**Calm nature**\n+SpD -Att";
        break;
      case "gentle":
        response = "**Gentle nature**\n+SpD -Def";
        break;
      case "sassy":
        response = "**Sassy nature**\n+SpD -Spe";
        break;
      case "careful":
        response = "**Careful nature**\n+SpD -SpA";
        break;
      case "quirky":
        response = "**Quirky nature**\nNeutral";
        break;
    }
    if (response != "") sendToServer(channelID, response);
    else
      sendToServer(
        channelID,
        "No Pokémon, ability, move, item or nature was found. Try again?"
      );
  }
}
...
function diceroll(channelID, message) {
  try {
    if (message.split(" ")[0].length != 2) {
      // if command word was 'roll' or 'dice
    ...
    var diceFlag = false,
      additionFlag = true,
      cutoffFlag = false;
    (finalString = ""), (refinedString = ""), (multiDiceCount = 0);
    for (var i = 0; i < message.length; i++) {
      ...
      switch (c) {
        ...
        case "9":
          if (!cutoffFlag) {
            if (!diceFlag) {
              // quantity is being set
              quantity += c;
            } else {
              size += c;
            }
          } else {
            // number of rolls to be cut off is being set
            cutoffNo += c;
            if (cutoffNo > quantity) {
              sendToServer(
                channelID,
                "You can't remove more dice than you roll!"
              );
              return;
            }
          }
        ...
        case "+":
          ...
          diceFlag
            ? (result = calculateRoll(quantity, size, cutoffNo))
        ...
        default:
          sendToServer(
            channelID,
            "Something wasn't right with that input. Try again?"
          );
          return;
      }
    }
...
function displayImage(channelID, message) {
  try {
    ...
    if (
      pokemonName == "urmum" ||
      pokemonName == "urmumlol" ||
      pokemonName == "urmom" ||
      pokemonName == "ur mum" ||
      pokemonName == "ur mum lol" ||
      pokemonName == "ur mom"
    ) {
      var fatMons = [
        "Guzzlord",
    ...
    switch (pokemonName.toLowerCase()) {
      case "owogull":
      case "uwugull":
        sendToServer(channelID, "https://i.imgur.com/pbFuWez.png");
        return;
        break;
      case "meltan":
        sendToServer(
          channelID,
          "http://assets.pokemon.com/assets/cms2/img/pokedex/full/808.png"
        );
        return;
      case "melmetal":
        sendToServer(
          channelID,
          "http://assets.pokemon.com/assets/cms2/img/pokedex/full/809.png"
        );
        return;
      case "jellicentf":
      case "jellicent-f":
      case "jellicent-female":
        sendToServer(
          channelID,
          "http://assets.pokemon.com/assets/cms2/img/pokedex/full/593_f2.png"
        );
        return;
      case "grookey":
        sendToServer(
          channelID,
          "https://swordshield.pokemon.com/assets/img/common/pokemon-1.png"
        );
        return;
      case "scorbunny":
        sendToServer(
          channelID,
          "https://swordshield.pokemon.com/assets/img/common/pokemon-2.png"
        );
        return;
      case "sobble":
        sendToServer(
          channelID,
          "https://swordshield.pokemon.com/assets/img/common/pokemon-3.png"
        );
        return;
    }
    ...
    if (isNaN(pokemonName)) {
      sqlImage =
        'SELECT species_id, form_order FROM PokemonForms INNER JOIN Pokemon ON PokemonForms.pokemon_id = Pokemon.id WHERE PokemonForms.identifier="' +
        pokemonName +
        '"';
    } else {
      sqlImage =
        'SELECT species_id, form_order FROM PokemonForms INNER JOIN Pokemon ON PokemonForms.pokemon_id = Pokemon.id WHERE PokemonForms.id="' +
        pokemonName +
        '"';
    }
...
function getMeme(sourceChannelID, channelID) {
  bot.getMessages(
    {
      channelID:
        sourceChannelID == "video"
          ? "436556584345272339"
          : "711595725477249024", // serverName party video-memeries or dnd-memes
      limit: 100,
      after:
        sourceChannelID == "video"
          ? pickGeneral(
              "436556752096329758 479043285458092033 509848646226018335 563648929523171338 618815435386650645 651111414307487773 674453303937204236"
            )
...
function getMeme2(sourceChannelID, channelID) {
  ...
  let numberOfRowsQuery =
    "SELECT COUNT(*) AS numberOfRows FROM " + sourceTable + ";";
  con.query(numberOfRowsQuery, function (err, result, fields) {
    // gets number of rows in respective table
    if (result[0]) {
      let savedMessageId = "";
      let numberOfRows = result[0].numberOfRows;
      let randomRow = Math.ceil(Math.random() * numberOfRows);
      let getMessageIdQuery =
        ...
        randomRow +
        ";";
      con.query(getMessageIdQuery, function (err, result2, fields) {
        // gets the messageID for the randomly selected row
        if (result2[0]) {
          ...
          bot.getMessage(
            ...
            function (err, res) {
              ...
              if (res) {
                ...
                // messageID not found in discord search - implies message has been deleted, id should be removed from the table
                sendToServer(
                  testChannelBugs,
                  "GetMeme error - messageID: " +
                    savedMessageId +
                    " was not found in discord search - remove from table: " +
                    sourceTable
                );
          ...
          // i.e. entry number not found - likely due to deletion - try again
...
function goddammitadmin(channelID) {
  ...
  function sendMeter() {
    ...
    sqlGoddammitUpdate =
      "UPDATE Goddammit SET number=" + goddammitadminMeter + ";";
...
function goddammitadminGeneral() {
  var responseArray = [
    " thinks you suck!",
    " reckons you're a second-rate reptile!",
    " thinks you need to be damned by a god!",
    " doesn't appreciate your little games!",
    " has never seen someone act so out of order as you just did!",
    " says ur mum lol",
    " pities the fool.",
    " is outraged by your shenanigans!",
  ];
...
function hangman(user, channelID, message) {
  ...
  if (!hangmanActive) {
    ...
    if (
      user != "Bazaro" &&
      (channelID == serverNamePartySpam || channelID == serverNamePartyGeneral)
    ) {
      hangmanActive = false;
      sendToServer(
        channelID,
        "Don't send me that here, silly! Message me privately so the others can't see!"
      );
      return;
    }
    try {
      ...
      sendToServer(
        channelID,
        'Please enclose your phrase with double quotes (").'
      );
      return;
    }
    if (word.length == 0) {
    ...
    if (illegalCharacterCheck) {
      sendToServer(
        channelID,
        "Please only use the 26 letters of the English alphabet."
      );
      return;
    }
    if (hangmanActive) {
      ...
      var introMessage =
        userNickname(user) +
        ' has started a game of hangman! Type *!hangman [letter]* to guess a letter, or *!hangman guess "[phrase]"* to guess the phrase itself. Type *!hangmancancel* to cancel the game.\nPhrase:  ' +
        blankedWord +
      ...
      if (channelID != serverNamePartySpam) {
        sendToServer(
          channelID,
          "Woo! Meet me over in botspam in serverName Party to watch the game!"
        );
      }
        ...
        try {
          if (word == message.split('"')[1].toLowerCase()) {
            var messageForGuesser =
              ...
              word +
              "**! Way to go!";
            for (var i = 0; i < wordLetters.length; i++) correctLetters[i] = 1;
          } else {
            var messageForGuesser =
              userNickname(user) +
              " incorrectly guessed **" +
            ...
            if (hangmanAttemptsLeft <= 0) {
              messageForGuesser +=
                '\n**Too bad! You ran out of guesses. The phrase was "' +
                word +
                '"!**';
          ...
          sendToServer(
            channelID,
            "Write your guess in double quotes please, " + userNickname(user)
          ) + "!";
        }
        return;
      }
    ...
    if (guessedLetter.length != 1) {
      sendToServer(
        channelID,
        "You can only enter one letter at a time, " + userNickname(user) + "!"
      );
      return;
    }
    ...
    if (incorrectLetters.includes(guessedLetter)) {
      sendToServer(
        channelID,
        "That letter has already been guessed, " + userNickname(user) + "!"
      );
      return;
    }
      ...
      if (guessedLetter == wordLetters[i]) {
        ...
        if (gameWon) {
          ...
          messageForGuesser +=
            ...
            '" is in the phrase!' +
            '\n**The game has been won! The phrase was "' +
            word +
            '"!**';
          updateHangmanStatus(messageForGuesser);
          resetGame();
...
function helixResponse(message) {
  if (message.split(" ")[1] != null) {
    var responseArray = [
      ...
      "Probably.",
      "Most likely.",
      'That\'s looking like a solid "Yes".',
      "Yep!",
      "Oh yes, definitely.",
      "My circuits are feeling tingly, maybe try again?",
      "Sorry, I got distracted. What?",
      "I don't know, ask " +
        pickGeneral(
          "Azery Bibs admin Prad Baz Njab Colin Ama Silje Arkhi Hycrox Aika Ice PT Lía"
        ) +
        ".",
      "Gimme a break, I don't know *everything*!",
      ...
      "That's less likely than admin getting out of bed.",
      "No way!",
      ...
      "Not a chance!",
      "Ha! Good one!",
      "I can't answer that, I wouldn't want to cause any drama.",
...
function helpResponse(channelID) {
  var helpString =
    "The Official Clarice Help Guide:" +
...
function learnMove(channelID, message) {
    ...
    case "alolan":
      inputPokemon =
        message
          .split(" ")[2]
          .toLowerCase()
          .substring(0, message.split(" ")[2].length - 1) + "-alola";
      break;
    case "mega":
      inputPokemon =
        message
          .split(" ")[2]
          .toLowerCase()
          .substring(0, message.split(" ")[2].length - 1) + "-mega";
      break;
  }

  var sqlMoveCheck =
    'SELECT Pokemon.identifier, identifierM, pokemon_move_method_id, level FROM Moves INNER JOIN (PokemonMoves INNER JOIN Pokemon ON PokemonMoves.pokemon_id = Pokemon.id) ON Moves.id = PokemonMoves.move_id WHERE (Pokemon.identifier="' +
    inputPokemon +
    '" AND identifierM="' +
    inputMove +
    '")';
  ...
  con.query(sqlMoveCheck, function (err, result, fields) {
    try {
      if (result.length != 0) {
        ...
        sendToServer(channelID, finalString);
      } else sendToServer(channelID, toTitleCase(inputPokemon) + " does not learn " + toTitleCase(inputMove) + ".");
    } catch (err) {
  ...
  function calculateResultDetails(i, result) {
    pokemon = result[i].identifier;
    move = result[i].identifierM;
    methodNo = result[i].pokemon_move_method_id;
    level = result[i].level;
    switch (methodNo) {
      case 0:
        method = " upon evolving.";
        break;
      case 1:
        method = " at level " + level + ".";
        break;
      case 2:
        method = " by breeding.";
        break;
      case 3:
        method = " by move tutor.";
        break;
      case 4:
        method = " by TM or HM.";
        break;
      default:
        method =
          " by some weird method admin didn't think it was necessary to program.";
        break;
    }
  }
}
...
function minesweeper(channelID, message) {
  ...
  if (message.split(" ")[1] != null) {
    try {
      ...
      if (gridSize < 5 || gridSize > 10) {
        sendToServer(
          channelID,
          "Grid size must be a number between between 5 and 10!"
        );
        return;
      }
    } catch (e) {
  ...
  for (
    var i = 0;
    i < mineLocationList.length;
    i++ // generate bomb locations
  ) {
    var complete = false;
    while (!complete) {
  ...
  mineLocationList.forEach(function (
    location // add bombs to grid
  ) {
        ...
        for (
          ...
          for (
            ...
            if (k >= 0 && k < gridSize && l >= 0 && l < gridSize) {
              // handles for corners and edges
              if (grid[k][l] == -1) bombCount++;
            }
          }
        }
        grid[i][j] = bombCount;
    ...
    for (var j = 0; j < gridSize; j++) {
      switch (grid[i][j]) {
        case -1:
          ...
          break;
        case 0:
          testGridString += "||:white_large_square:||";
          break;
        case 1:
          testGridString += "||:one:||";
          break;
        ...
        case 6:
          ...
          break;
        case 7:
          testGridString += "||:seven:||";
          break;
        case 8:
          testGridString += "||:eight:||";
          break;
        default:
          testGridString += "||:question:||";
          break;
      }
    }
    testGridString += "\n";
...
function movieList(channelID, message) {
  if (message.split(" ")[1]) {
    if (message.split(" ")[1].toLowerCase() == "add") {
      ...
      // remove existing movie from list
      let movieName = toTitleCase(message.split("remove ")[1]);
      console.log(movieName);
      let sqlAddMovie =
        "DELETE FROM movielist WHERE movie='" + movieName + "';";
      console.log(sqlAddMovie);
      con.query(sqlAddMovie, function (err, result, fields) {
      ...
      sendToServer(
        channelID,
        "Unknown argument - use 'add' or 'delete' next time!"
      );
    }
  } // no requests for command, just display the list
  else {
    let movies = "";
    ...
    con.query(sqlMovieList, function (err, result, fields) {
      try {
        ...
        } else sendToServer(channelID, "There are no movies in the list!");
      } catch (err) {
...
const dndRoleID = "<@&632058827042455583>",
  schedulingChannel = "797570049908408390";
const saturdayEmoji = "<:OMayaMoeShindeiru:836825404198879302>",
  sundayEmoji = "<:WhyThis:916421834331799634>";
function dndPing(channelID) {
  let pingMessage =
    dndRoleID +
    "\n**D&D is on the horizon!**\nWhich days are you available this weekend? React with " +
    saturdayEmoji +
    " for Saturday, " +
    sundayEmoji +
    " for Sunday.";
...
function nextBirthday(channelID, autoFlag) {
  ...
  let formattedCurrentDate =
    "'2000-" + (currentDate.getMonth() + 1) + "-" + currentDate.getDate() + "'";
  let sqlQuery =
  ...
  con.query(sqlQuery, function (err, result, fields) {
    ...
    if (result.length > 0) {
      ...
      if (
        monthName(result[0].birthday) === monthName(currentDate) &&
        dayName(result[0].birthday) === dayName(currentDate)
      ) {
        ...
        let subject =
          result[0].name == "Clarice" ? "my" : result[0].name + "'s";
        output += "Today is " + subject + " birthday!";
        if (
          result.length > 1 &&
          result[0].birthday.getTime() == result[1].birthday.getTime()
        ) {
          output += "\nIt's also " + result[1].name + "'s birthday!";
        }
      } else if (!autoFlag) {
        // stops command if doing daily birthday check
      ...
      // after the last listed birthday so no results
      sqlQuery =
        "SELECT * FROM Birthdays WHERE birthday>'2000-01-01' ORDER BY birthday ASC;";
      con.query(sqlQuery, function (err, result2, fields) {
        if (result2[0].birthday.toDateString() === currentDate.toDateString()) {
          // checks if today is a birthday
          output += "Today is " + result2[0].name + "'s birthday!";
          if (
            result2.length > 1 &&
            result2[0].birthday.getTime() == result[1].birthday.getTime()
          ) {
          ...
          if (
            result2.length > 1 &&
            result2[0].birthday.getTime() == result2[1].birthday.getTime()
          ) {
...
function njaaMessage(message) {
  ...
  for (var i = 0; i < messageFull.length; i++) {
    if (
      messageFull.substring(i, 1) == "r" ||
      messageFull.substring(i, 1) == "l"
    )
      njaaMessage += "w";
    else njaaMessage += messageFull.substring(i, 1);
  }
  return njaaMessage;
}

function randomGenerator(bmin, bmax, rmin, rmax, n) {
  // used for BST distribution for !cap
  ...
  do {
    sum = 0;
    for (i = 0; i < n; i++) sum += bmin + Math.random() * (bmax - bmin);
...
function randomPokemon(channelID) {
  var sqlRandom =
    'SELECT Pokemon.identifier FROM Pokemon WHERE species_id="' +
...
function removeMeme(channelID, message) {
  if (message.includes("dnd")) {
    ...
    let sqlRemoveDnd =
      'DELETE FROM pokemon.dndmemes WHERE messageID="' + messageID + '"';
    ...
    let messageID = message.split(" ")[2];
    let sqlRemoveVideo =
      'DELETE FROM pokemon.videomemes WHERE messageID="' + messageID + '"';
...
function pick(channelID, message) {
  try {
    ...
    if (options[randomNumber] == null)
      sendToServer(
        channelID,
        "You didn't give me anything to pick from, silly!"
      );
    else
...
function prune(userID, channelID, message) {
  if (
    userID == "540791878409256960" /* prad */ ||
    userID == "226459092598784004" /* Hycrox */ ||
    userID == adminID
  ) {
    let numToDelete = 0;
    if (message.split(" ")[1]) {
...
function pickGeneral(stringOptions) {
  return stringOptions.split(" ")[
    Math.floor(Math.random() * stringOptions.split(" ").length)
  ];
}
...
function smogonAnalysis(channelID, message) {
  try {
    ...
    if (metaValid) {
      var sqlSmogon =
        'SELECT identifier, generation_id FROM PokemonSpecies WHERE identifier="' +
        pokemonName +
        '"';
      con.query(sqlSmogon, function (err, result, fields) {
        if (result.length != 0) {
          if (result[0].generation_id <= genNo) {
            ...
            sendToServer(
              channelID,
              result[0].identifier + " is not in Gen " + genNo + "!"
            );
        } else sendToServer(channelID, "Pokémon not found. Try again?");
      });
    } else
      sendToServer(
        channelID,
        "Invalid meta specified. Try RB, GS, RS, DP, BW, XY or SM."
      );
  } catch (err) {
...
function timeConversion(channelID, message) {
  ...
  if (messageParts.length == 3) {
    // contains both reference person and time as expected
    let refPerson = messageParts[1].toLowerCase(); // person referenced in the original message, used to base starting time off of
    let refTimeContainer = messageParts[2].toLowerCase(),
      refTime,
      refMinute; // array to hold referenced time and minute (not yet implemented)
    if (!refTimeContainer.includes(":")) {
      refMinute = 0; // default
      refTime = parseInt(
        refTimeContainer.substr(0, refTimeContainer.indexOf("m") - 1)
      ); // substring up to am/pm, gets numbers only
      if (
        refTimeContainer.substr(refTimeContainer.length - 2) == "pm" &&
        refTime != 12
      ) {
        refTime += 12;
      }
      if (
        refTimeContainer.substr(refTimeContainer.length - 2) == "am" &&
        refTime == 12
      ) {
        // 12am i.e. midnight
        refTime -= 12;
      ...
      switch (refPerson) {
        ...
        case "nick":
          utcMod = 5.0;
          break; // CHANGE THESE WHEN DST OCCURS + see timezoneMods below ↴
        case "baz":
        case "bazaro":
        case "jon":
          utcMod = 4.0;
        ...
        case "ironbound":
          ...
          break;
        default:
          sendToServer(channelID, "Name not found. Try again?");
          return;
      }
      ...
      for (var i = 0; i < adjustedTimes.length; i++) {
        ...
        if (tempTime >= 0.0 && tempTime <= 12) {
          ...
          if (tempTime != 12.0) tempTime %= 12;
        }
        if (Number.isInteger(tempTime)) {
          // for whole hour times
          if (tempTime == 0.0)
            // adjusts 0am to 12am
            tempTime = 12.0;
          ...
          // for half hour times
          if (tempTime == 0.0 || tempTime == 0.5) tempTime = 12.0; // adjusts 0am to 12am
          adjustedTimes[i] =
            tempTime.toString().split(".")[0] + ":30" + meridian;
        }
      }
      let finalString =
        "**" +
        ...
        adjustedTimes[2] +
        "\nEte/Lía: " +
        adjustedTimes[3]; /*INDIA + "\nPrad: " + adjustedTimes[4]*/
      sendToServer(channelID, finalString);
    } else {
      sendToServer(
        channelID,
        "Time conversions must be formatted as !time [person] [integer][am/pm]."
      );
      return;
    }

    let winnipegModifier = -6.0,
      ohioModifier = -5.0,
      ukModifier = 0.0,
      europeModifier = 1.0,
      indiaModifier = 5.5;
...
function uwuMessage(message, isForPrevious) {
  ...
  for (var i = 0; i < messageFull.length; i++) {
    if (
      messageFull.substring(i, i + 1) == "r" ||
      messageFull.substring(i, i + 1) == "l"
    )
      uwuMessage += "w";
    else if (
      messageFull.substring(i, i + 1) == "R" ||
      ...
      uwuMessage += "W";
    else uwuMessage += messageFull.substring(i, i + 1);
  }
  return uwuMessage;
}
...
function weaknessCheck(channelID, message) {
  ...
  con.query(sqlTypeNo, function (err, result, fields) {
    try {
      if (result.length != 0) {
        typeNo = result[0].id;
        var sqlWeaknessType =
          "SELECT identifier, damage_factor FROM Types INNER JOIN TypeEffectiveness ON Types.id = TypeEffectiveness.damage_type_id WHERE target_type_id=" +
          typeNo;
        con.query(sqlWeaknessType, function (err, result, fields) {
          var matchupString =
            "**" + toTitleCase(input) + " Type Defensive Matchups**\nWeak:   ";
          var damageFactors = new Array(18);
          for (var i = 0; i < result.length; i++) {
            damageFactors[i] = result[i].damage_factor;
          }
          if (message.split(" ")[2] != null) {
            // if second type was specified
            var sqlTypeNo =
              'SELECT id FROM Types WHERE identifier="' +
            ...
            con.query(sqlTypeNo, function (err, result, fields) {
              if (result.length != 0) {
                ...
                con.query(sqlWeaknessType, function (err, result, fields) {
                  if (result.length != 0) {
                    ...
                    var inputTypesString =
                      toTitleCase(message.split(" ")[1]) +
                      " / " +
                      toTitleCase(message.split(" ")[2]);
                    var matchupString =
                      "**" +
                      inputTypesString +
                      " Defensive Matchups**\nWeak (4x):   ";
                    ...
                    if (!weak4Found) matchupString += "None";
                    matchupString += "\nWeak (2x):   ";
                    ...
                    if (!weak2Found) matchupString += "None";
                    matchupString += "\nResists (2x):   ";
                    ...
                    if (!resist2Found) matchupString += "None";
                    matchupString += "\nResists (4x):   ";
                    ...
                    if (!resist4Found) matchupString += "None";
                    matchupString += "\nImmune:   ";
                    ...
                    if (!immuneFound) matchupString += "None";
                ...
                sendToServer(
                  channelID,
                  "Something was wrong with that input. Try again?"
                );
              }
            });
          } else {
      ...
      } // if pokemon name was entered rather than type name
      else pokemonWeaknessCheck();
    } catch (err) {
  ...
  function pokemonWeaknessCheck() {
    ...
    var sqlWeaknessPokemon =
      'SELECT type_id FROM Pokemon INNER JOIN PokemonTypes ON Pokemon.id = PokemonTypes.pokemon_id WHERE identifier="' +
      input +
      '"';
    con.query(sqlWeaknessPokemon, function (err, result, fields) {
      try {
        if (result.length != 0) {
          ...
          function attemptType2Query() {
            if (result.length > 1) {
              ...
              for (var i = 0; i < 18; i++) type2Matchups[i] = 100;
              finishWeaknessString();
            }
          }
        ...
        } else sendToServer(channelID, "No type or Pokémon was found. Try again?");
      } catch (err) {
        sendToServer(
          testChannelBugs,
          "Error in function: pokemonWeaknessCheck"
        );
        sendToServer(testChannelBugs, err);
      }
    });
...
function wordle(channelID, message, user) {
  ...
  if (wordleGameInProgress) {
    ...
    if (!message.split(" ")[1]) {
      sendToServer(
        channelID,
        "There's already a Wordle game in progress. Wait for it to finish before starting a new one. To cancel the current game, type ``!wordle cancel``."
      );
      return;
    } else {
      // an input has been entered
      let input = message.split(" ")[1].toLowerCase();
      // check if cancelling game
      if (input == "cancel") {
        sendToServer(
          channelID,
          `Game cancelled. The correct answer was **${correctAnswer}**.`
        );
      ...
      if (input == "who") {
        sendToServer(
          channelID,
          "The current player is " + userNickname(currentPlayer) + "!"
        );
        return;
      }
      if (input == "letters") {
      ...
      if (user != currentPlayer) {
        sendToServer(
          channelID,
          userNickname(currentPlayer) +
            " is currently playing! Wait for them to finish, or type ``!wordle cancel`` to stop the current game."
        );
        return;
      }
      ...
      if (!input.match("[a-z]{5}")) {
        sendToServer(
          channelID,
          "Your guess must only contain English letters!"
        );
        return;
      }
      if (
        !wordleValidGuesses.includes(input) &&
        !wordleValidAnswers.includes(input)
      ) {
      ...
      for (var i = 0; i < correctAnswer.length; i++) {
        if (correctAnswer.includes(input.substring(i, i + 1))) {
          // letter matches
          if (correctAnswer.substring(i, i + 1) == input.substring(i, i + 1)) {
            // position matches
        ...
        function getWordleEmoji(colour, letter) {
          ...
          switch (colour) {
            case "green":
              switch (letter) {
                case "a":
                  emojiID = "938084998664314920";
                  break;
                case "b":
                  emojiID = "938085009829548032";
                  break;
                case "c":
                  emojiID = "938085060526112808";
                  break;
                case "d":
                  emojiID = "938085071313862816";
                  break;
                case "e":
                  emojiID = "938085081216589834";
                  break;
                case "f":
                  emojiID = "938085089634553937";
                  break;
                case "g":
                  emojiID = "938085098555834508";
                  break;
                case "h":
                  emojiID = "938085108370505728";
                  break;
                case "i":
                  emojiID = "938085117853859891";
                  break;
                case "j":
                  emojiID = "938085127492354110";
                  break;
                case "k":
                  emojiID = "938085136996663346";
                  break;
                case "l":
                  emojiID = "938085145594982470";
                  break;
                case "m":
                  emojiID = "938085153677385790";
                  break;
                case "n":
                  emojiID = "938085161852104717";
                  break;
                case "o":
                  emojiID = "938085182731337779";
                  break;
                case "p":
                  emojiID = "938085193997238292";
                  break;
                case "q":
                  emojiID = "938085203686064189";
                  break;
                case "r":
                  emojiID = "938085214415118366";
                  break;
                case "s":
                  emojiID = "938085222241693776";
                  break;
                case "t":
                  emojiID = "938085231938908221";
                  break;
                case "u":
                  emojiID = "938085326533066803";
                  break;
                case "v":
                  emojiID = "938085336817471489";
                  break;
                case "w":
                  emojiID = "938085350792888349";
                  break;
                case "x":
                  emojiID = "938085360855035935";
                  break;
                case "y":
                  emojiID = "938085370896199703";
                  break;
                case "z":
                  emojiID = "938085390458421268";
                  break;
              }
              break;
            case "yellow":
              switch (letter) {
                case "a":
                  emojiID = "938090629253185637";
                  break;
                case "b":
                  emojiID = "938090639340503050";
                  break;
                case "c":
                  emojiID = "938090648492458014";
                  break;
                case "d":
                  emojiID = "938090660282630156";
                  break;
                case "e":
                  emojiID = "938090671863128124";
                  break;
                case "f":
                  emojiID = "938090681891688528";
                  break;
                case "g":
                  emojiID = "938090691890933791";
                  break;
                case "h":
                  emojiID = "938090716951883826";
                  break;
                case "i":
                  emojiID = "938090727815147551";
                  break;
                case "j":
                  emojiID = "938090737562693712";
                  break;
                case "k":
                  emojiID = "938090747037614110";
                  break;
                case "l":
                  emojiID = "938090757112352868";
                  break;
                case "m":
                  emojiID = "938090768101425182";
                  break;
                case "n":
                  emojiID = "938090778490699856";
                  break;
                case "o":
                  emojiID = "938090788875808889";
                  break;
                case "p":
                  emojiID = "938090799424503809";
                  break;
                case "q":
                  emojiID = "938090809822175312";
                  break;
                case "r":
                  emojiID = "938090819259359253";
                  break;
                case "s":
                  emojiID = "938090830382657626";
                  break;
                case "t":
                  emojiID = "938090840901951540";
                  break;
                case "u":
                  emojiID = "938090850557231145";
                  break;
                case "v":
                  emojiID = "938090861336616960";
                  break;
                case "w":
                  emojiID = "938090883331555348";
                  break;
                case "x":
                  emojiID = "938090903556476979";
                  break;
                case "y":
                  emojiID = "938090914063187988";
                  break;
                case "z":
                  emojiID = "938090925865988126";
                  break;
              }
              break;
            case "grey":
              switch (letter) {
                case "a":
                  emojiID = "938085441310175303";
                  break;
                case "b":
                  emojiID = "938085450785120286";
                  break;
                case "c":
                  emojiID = "938085468476670042";
                  break;
                case "d":
                  emojiID = "938085490949750834";
                  break;
                case "e":
                  emojiID = "938085500613447711";
                  break;
                case "f":
                  emojiID = "938085546461368391";
                  break;
                case "g":
                  emojiID = "938085558704558151";
                  break;
                case "h":
                  emojiID = "938085575796338760";
                  break;
                case "i":
                  emojiID = "938085586063995001";
                  break;
                case "j":
                  emojiID = "938085596822388777";
                  break;
                case "k":
                  emojiID = "938085608218329139";
                  break;
                case "l":
                  emojiID = "938085619249324032";
                  break;
                case "m":
                  emojiID = "938085629609250876";
                  break;
                case "n":
                  emojiID = "938085643047825468";
                  break;
                case "o":
                  emojiID = "938085655454572575";
                  break;
                case "p":
                  emojiID = "938085666871472239";
                  break;
                case "q":
                  emojiID = "938085677583716382";
                  break;
                case "r":
                  emojiID = "938085688237256744";
                  break;
                case "s":
                  emojiID = "938085697984790619";
                  break;
                case "t":
                  emojiID = "938085707480694784";
                  break;
                case "u":
                  emojiID = "938085716670423111";
                  break;
                case "v":
                  emojiID = "938085726128590879";
                  break;
                case "w":
                  emojiID = "938085735557378098";
                  break;
                case "x":
                  emojiID = "938085744600301608";
                  break;
                case "y":
                  emojiID = "938090605643456552";
                  break;
                case "z":
                  emojiID = "938090616833835038";
                  break;
              }
              break;
          }
      ...
      if (input == correctAnswer) {
        ...
        output += `\n${comment}! You won in ${
          wordleGuessesLeft == 6 ? "a" : 7 - wordleGuessesLeft
        } ${
          wordleGuessesLeft == 6 ? "single guess" : "guesses"
        }! The correct answer was **${toTitleCase(correctAnswer)}**!`;
        sendToServer(channelID, output);
        wordleGameInProgress = false;
        return;
      }
      wordleGuessesLeft--;
      if (wordleGuessesLeft == 0) {
        // lose
        output = cumulativeOutput + "\n" + output;
    ...
    if (wordleValidAnswers == "")
      wordleValidAnswers = fs
        .readFileSync("wordle-answers.txt", "utf-8")
        .split("\n");
    if (wordleValidGuesses == "")
      wordleValidGuesses = fs
        .readFileSync("wordle-guesses.txt", "utf-8")
        .split("\n");
    correctAnswer =
      wordleValidAnswers[Math.floor(Math.random() * wordleValidAnswers.length)];
    let output = `**${userNickname(
      user  
